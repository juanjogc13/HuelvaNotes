    ...
    <div class="max-w-5xl mx-auto px-6 py-10">
        ...
        <div class="mb-10">
            <h2 class="text-3xl font-black tracking-tighter">
                Mi <span class="text-orange-500">Perfil</span>
            </h2>
        ...
        @if(session('status') === 'perfil-actualizado')
            <div class="bg-green-500/10 border border-green-500/30 rounded-2xl p-4 mb-6 flex items-center gap-3">
                ...
                <p class="text-green-400 text-sm font-bold">Perfil actualizado correctamente.</p>
            </div>
        @endif
        ...
        @if(session('status') === 'password-actualizado')
            <div class="bg-green-500/10 border border-green-500/30 rounded-2xl p-4 mb-6 flex items-center gap-3">
                <i class="bi bi-check-circle-fill text-green-400 text-lg"></i>
                <p class="text-green-400 text-sm font-bold">Contraseña actualizada correctamente.</p>
            </div>
        @endif
        ...
        @if(session('status') === 'apunte-eliminado')
            <div class="bg-green-500/10 border border-green-500/30 rounded-2xl p-4 mb-6 flex items-center gap-3">
                <i class="bi bi-check-circle-fill text-green-400 text-lg"></i>
                <p class="text-green-400 text-sm font-bold">Apunte eliminado correctamente.</p>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white/5 border border-white/10 rounded-3xl p-8 text-center">
                    ...
                    @if(in_array($user->rol, ['admin', 'moderador']))
                        <div class="mt-3 inline-flex items-center gap-2 px-3 py-1 rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-400 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-white font-bold flex items-center gap-1">
                                {{ number_format($valoracionMedia, 1) }}
                ...
                {{-- Ranking --}}
                <a href="{{ route('ranking.index') }}"
                    class="block bg-orange-500/10 border border-orange-500/30 rounded-3xl p-6 hover:bg-orange-500/15 transition">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <h3 class="text-[10px] font-bold text-orange-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                                <i class="bi bi-trophy-fill"></i>
                                Ranking
                            </h3>
                            <p class="text-white font-black text-lg">Ver clasificación</p>
                            <p class="text-white/40 text-xs mt-1">Compara tus puntos con otros usuarios.</p>
                        </div>

                        <i class="bi bi-arrow-right text-orange-500 text-xl"></i>
                    </div>
                </a>
                ...
                <div class="bg-white/5 border border-white/10 rounded-3xl p-6">
                    ...
                    @forelse($historialPuntos as $t)
                        <div class="flex justify-between items-center py-2 border-b border-white/5">
                            <span class="text-white/40 text-xs capitalize flex items-center gap-2">
                                @if($t->cantidad > 0)
                                    <i class="bi bi-plus-circle-fill text-green-400"></i>
                                @else
            ...
            <div class="lg:col-span-2 space-y-4">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-5">
                        ...
                        <div x-data="{
                            ...
                            <button type="button" @click="open = !open"
                                ...
                                <i class="bi bi-chevron-down absolute right-5 top-1/2 -translate-y-1/2 text-white/30 text-sm"></i>
                            </button>
                                ...
                                <div class="p-3 border-b border-white/10">
                                    <div class="relative">
                                        ...
                                        <input type="text" x-model="search" placeholder="Busca tu centro..."
                                            class="w-full pl-11 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-white/20 focus:outline-none">
                                    </div>
                                </div>
                        ...
                        <button type="submit"
                            class="w-full py-4 bg-orange-600 text-white font-black rounded-2xl hover:bg-orange-500 transition uppercase tracking-widest text-xs flex items-center justify-center gap-2">
                    ...
                    <form method="POST" action="{{ route('profile.password') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="password" name="password" required
                                class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white focus:outline-none focus:border-orange-500/50 transition-all duration-300"
                                placeholder="••••••••">
                            <p class="text-white/20 text-[10px] mt-1 ml-1">
                        ...
                        <button type="submit"
                            class="w-full py-4 bg-white/10 text-white font-black rounded-2xl hover:bg-white/20 transition uppercase tracking-widest text-xs flex items-center justify-center gap-2">
                ...
                <div class="bg-white/5 border border-white/10 rounded-3xl p-8">
                    ...
                    @forelse($apuntes as $apunte)
                        <div class="flex justify-between items-center border-b border-white/10 py-3 gap-4">
                            <div>
                            ...
                            <form method="POST" action="{{ route('apuntes.destroy', $apunte->id) }}">
                                ...
                                <button type="submit" onclick="return confirm('¿Seguro que quieres eliminar este apunte?')"
                                    class="text-red-400 hover:text-red-300 text-xs font-bold uppercase tracking-widest transition flex items-center gap-2">
                    ...
                    @forelse($favoritos as $favorito)
                        <div class="border-b border-white/10 py-3">
                            <p class="text-white font-semibold text-sm">
                                {{ $favorito->apunte->titulo ?? 'Apunte eliminado' }}
                            </p>
                            <p class="text-white/30 text-xs mt-1">
                                {{ $favorito->created_at->diffForHumans() }}
                            </p>
                        </div>
                    @empty
                        <p class="text-white/30 text-sm">
                            No tienes apuntes en favoritos.
                        </p>
                    @endforelse
                </div>
                ...
                <div class="bg-red-500/5 border border-red-500/20 rounded-3xl p-8">
                    <h3 class="text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                        <i class="bi bi-exclamation-triangle-fill"></i>
                        Zona de peligro
                    </h3>

                    <p class="text-white/30 text-sm mb-6">
                        Si eliminas tu cuenta perderás todos tus apuntes, puntos y datos. Esta acción no se puede deshacer.
                    </p>
                    ...
                    <form method="POST" action="{{ route('profile.destroy') }}" onsubmit="return confirm('¿Estás seguro? Esta acción no se puede deshacer.')">
                        ...
                        <div class="mb-4">
                            <label class="block text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2 ml-1">
                                Confirma tu contraseña
                            </label>
                            <input type="password" name="password" required
                                class="w-full px-6 py-4 bg-white/5 border border-red-500/20 rounded-2xl text-white focus:outline-none focus:border-red-500/50 transition-all duration-300"
                                placeholder="••••••••">
                        </div>

                        <button type="submit"
                            class="w-full py-4 bg-red-600 text-white font-black rounded-2xl hover:bg-red-500 transition uppercase tracking-widest text-xs flex items-center justify-center gap-2">
                            <i class="bi bi-trash-fill"></i>
                            Eliminar mi cuenta
                        </button>
                    </form>
