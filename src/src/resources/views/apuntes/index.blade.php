                        ...
                        @foreach($apuntes as $apunte)
                            ...
                            <div x-data="{ open: false }">
                                ...
                                <div x-show="open"
                                    ...
                                    {{-- Contenido del modal con scroll si hay mucho contenido --}}
                                    <div class="relative bg-gray-950 border border-white/10 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl"
                                        ...
                                        {{-- Cabecera del modal con formato, fecha y título --}}
                                        ...
                                        <div class="p-8 space-y-6">
                                            ...
                                            {{-- Grid con asignatura, curso, nivel y centro --}}
                                            ...
                                            {{-- Información del autor con foto o inicial --}}
                                            ...
                                            {{-- Botón de descarga con distintos estados --}}
                                            <div>
                                                @if($apunte->user_id === Auth::id())
                                                    {{-- Es tu propio apunte, no puedes descargarlo --}}
                                                ...
                                                @elseif($user->descargas()->where('apunte_id', $apunte->id)->exists())
                                                    {{-- Ya descargado, puede volver a descargarlo gratis --}}
                                                ...
                                                @elseif($user->puntos >= $apunte->coste_puntos)
                                                    {{-- Tiene puntos suficientes para descargar --}}
                                                    <form method="POST" action="{{ route('apuntes.download', $apunte->id) }}" @click.stop>
                                                        @csrf
                                            ...
                                            @if($apunte->user_id !== Auth::id() && $user->descargas()->where('apunte_id', $apunte->id)->exists())
                                                @php
                                                    // Comprobamos si el usuario ya valoró este apunte
                                                ...
                                                @if(!$yaValorado)
                                                    <div class="bg-white/5 rounded-2xl p-5">
                                                        ...
                                                        <form method="POST" action="{{ route('apuntes.valorar', $apunte->id) }}" @click.stop x-data="{ stars: 0 }">
                                                            @csrf
                                                            {{-- Las estrellas interactivas con Alpine.js --}}
                                                ...
                                                @else
                                                    {{-- Ya ha valorado, mostramos confirmación --}}
                                            ...
                                            @elseif($apunte->user_id !== Auth::id())
                                                {{-- No ha descargado el apunte, no puede valorar --}}
